...
func convertBSP3toScene(file_path : String) -> Node:
	clear_data()
	print("Attempting to import Q3 BSP: ", file_path)
	file = FileAccess.open(file_path, FileAccess.READ)
	if !file:
		error = FileAccess.get_open_error()
		print("Failed to open %s: %d" % [file_path, error])
		return null
	...
	# Read Entities
	var entity_bytes = get_lump_data(lumps, LUMP_ENTITIES)
	var entity_string = entity_bytes.get_string_from_utf8()
	var entity_dict_array = parse_entity_string(entity_string)

	# Read Shaders/Textures
	# Each shader: 64-byte name + 4-byte flags + 4-byte contents = 72 bytes total
	var shader_bytes: PackedByteArray = get_lump_data(lumps, LUMP_SHADERS)
	var shader_count = shader_bytes.size() / 72
	var shaders = []
	for i in range(shader_count):
		var base = i * 72
		var name_bytes = shader_bytes.slice(base, 64)
		var shader_name = name_bytes.get_string_from_utf8().rstrip("\\x00").lstrip("\\x00")
		var flags = shader_bytes.decode_s32(base + 64)
		var contents = shader_bytes.decode_s32(base + 68)
		shaders.append({
			"name": shader_name,
			"flags": flags,
			"contents": contents
	...
	# Read Vertices
	# Q3 Vertex: position(3 floats) + uv(2 floats) + lm_uv(2 floats) + normal(3 floats) + color(4 bytes) = 44 bytes
	# position:12 bytes, uv:8 bytes, lm_uv:8 bytes, normal:12 bytes, color:4 bytes
	var vertex_bytes: PackedByteArray = get_lump_data(lumps, LUMP_VERTICES)
	var vertex_size = 44
	var vertex_count = vertex_bytes.size() / vertex_size
	var q3_vertices = []
	q3_vertices.resize(vertex_count)
	for i in range(vertex_count):
		var base = i * vertex_size
		var x = vertex_bytes.decode_float(base)
		var y = vertex_bytes.decode_float(base + 4)
		var z = vertex_bytes.decode_float(base + 8)
		var u = vertex_bytes.decode_float(base + 12)
		var v = vertex_bytes.decode_float(base + 16)
		var lm_u = vertex_bytes.decode_float(base + 20)
		var lm_v = vertex_bytes.decode_float(base + 24)
		var nx = vertex_bytes.decode_float(base + 28)
		var ny = vertex_bytes.decode_float(base + 32)
		var nz = vertex_bytes.decode_float(base + 36)
		# color RGBA (4 bytes)
		var cr = vertex_bytes[base + 40]
		var cg = vertex_bytes[base + 41]
		var cb = vertex_bytes[base + 42]
		var ca = vertex_bytes[base + 43]

		var pos = convert_vector_from_quake_unscaled(Vector3(x,y,z))
		var normal = convert_vector_from_quake_unscaled(Vector3(nx, ny, nz)).normalized()
		q3_vertices[i] = {
			"pos": pos,
			"uv": Vector2(u,v),
			"lm_uv": Vector2(lm_u, lm_v),
			"normal": normal,
			"color": Color(cr/255.0,cg/255.0,cb/255.0,ca/255.0)
		}

	# Meshverts
	# Q3 MeshVert: 4 bytes (int)
	var meshvert_bytes = get_lump_data(lumps, LUMP_MESHVERTS)
	var meshvert_count = meshvert_bytes.size() / 4
	var meshverts = PackedInt32Array()
	meshverts.resize(meshvert_count)
	for i in range(meshvert_count):
		meshverts[i] = meshvert_bytes.decode_s32(i*4)

	# Faces
	# Q3 Face (for version 46): 104 bytes structure
	# struct face {
	#   int texture, int effect, int type;
	#   int vertex, n_vertex;
	#   int meshvert, n_meshverts;
	#   int lm_index;
	...
	#   int size[2];
	# }
	var face_bytes: PackedByteArray = get_lump_data(lumps, LUMP_FACES)
	var face_size = 104
	var face_count = face_bytes.size() / face_size
	var faces = []
	for i in range(face_count):
		var base = i * face_size
		var texture_id = face_bytes.decode_s32(base + 0)
		var effect_id = face_bytes.decode_s32(base + 4)
		var face_type = face_bytes.decode_s32(base + 8)
		var first_vert = face_bytes.decode_s32(base + 12)
		var num_verts = face_bytes.decode_s32(base + 16)
		var first_meshvert = face_bytes.decode_s32(base + 20)
		var num_meshverts = face_bytes.decode_s32(base + 24)
		var lm_index = face_bytes.decode_s32(base + 28)
		# lightmap start
		var lm_startx = face_bytes.decode_s32(base + 32)
		var lm_starty = face_bytes.decode_s32(base + 36)
		var lm_sizex = face_bytes.decode_s32(base + 40)
		var lm_sizey = face_bytes.decode_s32(base + 44)
		var lm_origin_x = face_bytes.decode_float(base + 48)
		var lm_origin_y = face_bytes.decode_float(base + 52)
		var lm_origin_z = face_bytes.decode_float(base + 56)
		var lm_vecs = [
			Vector3(face_bytes.decode_float(base+60), face_bytes.decode_float(base+64), face_bytes.decode_float(base+68)),
			Vector3(face_bytes.decode_float(base+72), face_bytes.decode_float(base+76), face_bytes.decode_float(base+80))
		]
		var normal_x = face_bytes.decode_float(base+84)
		var normal_y = face_bytes.decode_float(base+88)
		var normal_z = face_bytes.decode_float(base+92)
		var size_x = face_bytes.decode_s32(base+96)
		var size_y = face_bytes.decode_s32(base+100)

		var normal = convert_vector_from_quake_unscaled(Vector3(normal_x,normal_y,normal_z)).normalized()
		faces.append({
			...
			"num_verts": num_verts,
			"first_meshvert": first_meshvert,
			"num_meshverts": num_meshverts,
			"normal": normal
		})
	...
	# Build geometry
	# Similar approach as Q2: build a surface tool per shader
	var shader_tools = {}
	for s in shaders:
		var st = SurfaceTool.new()
		st.begin(Mesh.PRIMITIVE_TRIANGLES)
		var mat_info = load_or_create_material(s.name)
		st.set_material(mat_info.material)
		shader_tools[s.name] = st

	for face in faces:
		var ftype = face["type"]
		# Q3 Face Types:
		# 1 = Polygon, 2 = Patch, 3 = Mesh, 4 = Billboard
		# We'll handle polygon and mesh
		if ftype == 1 or ftype == 3:
			var st = shader_tools[ shaders[face["texture_id"]].name ]
			var base_vert = face["first_vert"]
			var n_verts = face["num_verts"]
			var base_meshvert = face["first_meshvert"]
			var n_meshverts = face["num_meshverts"]

			# The indexing of Q3 faces is by 'meshverts' for triangulation
			# We'll build triangles using meshverts as indices into vertex arrays
			for i in range(n_meshverts/3):
				var idx0 = meshverts[base_meshvert + i*3 + 0]
				var idx1 = meshverts[base_meshvert + i*3 + 1]
				var idx2 = meshverts[base_meshvert + i*3 + 2]
				var v0 = q3_vertices[base_vert + idx0]
				var v1 = q3_vertices[base_vert + idx1]
				var v2 = q3_vertices[base_vert + idx2]

				# Add triangle
				st.set_normal(v0.normal)
				st.set_color(v0.color)
				st.set_uv(v0.uv)
				st.add_vertex(v0.pos)

				st.set_normal(v1.normal)
				st.set_color(v1.color)
				st.set_uv(v1.uv)
				st.add_vertex(v1.pos)

				st.set_normal(v2.normal)
				st.set_color(v2.color)
				st.set_uv(v2.uv)
				st.add_vertex(v2.pos)

		else:
			# Skipping patches and billboards for simplicity
			pass

	# Commit surfaces
	var array_mesh = ArrayMesh.new()
	for s in shaders:
		var st = shader_tools[s.name]
		st.generate_tangents()
		array_mesh = st.commit(array_mesh)

	var mesh_instance = MeshInstance3D.new()
	mesh_instance.mesh = array_mesh
	mesh_instance.name = "Q3Geometry"
	root_node.add_child(mesh_instance)
	mesh_instance.owner = root_node

	# Place Entities (already done in convert_entity_dict_to_scene)

	run_post_import_script()

	file.close()
	file = null
	print("Q3 BSP read complete.")
	return root_node
